...
router.post("/scan", async (req, res) => {
  ...
  try {
    const users = await db.select().from(usersTable).where(eq(usersTable.uniqueId, uniqueId)).limit(1);
    if (!users[0]) {
    ...
    if (!existingRecords[0]) {
      ...
      res.json({
        ...
        user: { id: user.id, name: user.name, uniqueId: user.uniqueId, role: user.role, createdAt: user.createdAt.toISOString() },
