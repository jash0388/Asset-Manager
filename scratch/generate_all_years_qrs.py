...
# Define folder mappings for batches
# Batch 2025 (Year 2) -> 1st_Year
# Batch 2024 (Year 3) -> 2nd_Year
# Batch 2023 (Year 4) -> 3rd_Year
YEAR_FOLDERS = {
    "2025": "1st_Year",
    "2024": "2nd_Year",
    "2023": "3rd_Year"
...
# 1. Parse Excel Sheet and Group by Batch/Year
...
for sheet_name in wb.sheetnames:
    ...
    for row in ws.iter_rows(values_only=True):
        ...
        # Check for batch header to identify year
        if "Batch:" in row_str:
            match = re.search(r"Batch:\s*(\d+)", row_str, re.IGNORECASE)
            if match:
                batch_year = match.group(1).strip()
                current_batch = YEAR_FOLDERS.get(batch_year, None)
            continue
        ...
        # Parse data row
        if current_batch and ht_col is not None and name_col is not None:
            ...
            years_data[current_batch].append({
                "name": name_str,
                "roll_no": ht_str
            })

print("   ✓ Finished parsing all student years.")
for yr, studs in years_data.items():
    print(f"     - {yr}: {len(studs)} students")
...
for year_name, students in years_data.items():
    if not students:
        continue
    
    print(f"\n🎨 Processing Year Category: {year_name} ({len(students)} students)")
    year_png_dir = os.path.join(OUTPUT_DIR, year_name)
    os.makedirs(year_png_dir, exist_ok=True)
    
    # Sort students by roll number for structured PDF output
    ...
    for s in students:
        ...
        png_path = os.path.join(year_png_dir, f"{safe_roll}.png")
        ...
        all_created_items.append((png_path, os.path.join(year_name, f"{safe_roll}.png")))
        
    # 2. Generate year PDF file
    pdf_path = os.path.join(OUTPUT_DIR, f"{year_name}_printable.pdf")
    pdf_c = canvas.Canvas(pdf_path, pagesize=A4)
    pdf_c.setTitle(f"QR Codes - {year_name.replace('_', ' ')}")
    ...
    while idx < total:
        ...
        pdf_c.drawCentredString(PAGE_W / 2, 6 * mm, f"QR Year Category: {year_name.replace('_', ' ')} — Page {pdf_c.getPageNumber()}")
    ...
    all_created_items.append((pdf_path, f"{year_name}_printable.pdf"))
...
print("🎉 ZIP package with all 3 years of student QRs generated successfully!")
