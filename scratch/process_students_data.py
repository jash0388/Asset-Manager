...
# SSL configuration for script requests
...
OUTPUT_PNG_DIR = "student_qr_codes"
OUTPUT_PDF_DIR = "scratch/printable_qrs"
...
sections_data = {}  # Format: { "DS_IV-I-A": [ {name, roll_no}, ... ] }
...
for sheet_name in wb.sheetnames:
    ...
    for row in ws.iter_rows(values_only=True):
        ...
        # Check for table header row to detect columns
        ...
        # Parse data row if section is known and columns are found
...
for sec, studs in sections_data.items():
    print(f"   Section {sec}: parsed {len(studs)} students")
    total_parsed += len(studs)
    
    # Reconstruct original section name, e.g. "DS IV-I-A"
    display_sec = sec.replace("_", " ").replace("-", "/")
...
print(f"\n📊 Total students parsed successfully: {total_parsed}")
if total_parsed == 0:
    print("❌ No students parsed. Check headers and section markers in Excel sheet.")
    sys.exit(1)

# ── 2. Insert/Upsert into Supabase ───────────────────────────────────────────
print(f"\n🚀 Inserting {len(all_students)} students into Supabase `qr_users` table...")
url = f"{SUPABASE_URL}/rest/v1/qr_users?on_conflict=unique_id"

# Supabase REST API bulk insert with upsert
# Chunking requests to avoid payload size limits
chunk_size = 100
for i in range(0, len(all_students), chunk_size):
    chunk = all_students[i:i + chunk_size]
    ...
    try:
        with urllib.request.urlopen(req, context=ssl_ctx) as resp:
            print(f"   ✓ Uploaded chunk {i // chunk_size + 1}/{(len(all_students) - 1) // chunk_size + 1}")
...
# ── 3. Generate Individual PNG QR Codes and Section PDF sheets ────────────────
print("\n🎨 Generating QR codes and printable PDFs section-wise...")

all_created_files = []

# reportlab layout config
PAGE_W, PAGE_H = A4
COLS      = 4
MARGIN    = 10 * mm
QR_SIZE   = 42 * mm
...
for section_name, students in sections_data.items():
    ...
    sec_png_dir = os.path.join(OUTPUT_PNG_DIR, section_name)
    ...
    for s in students:
        ...
        # Create individual QR code image
        qr = qrcode.QRCode(version=1, box_size=10, border=3)
        # Encode clean roll number
        qr.add_data(roll)
        qr.make(fit=True)
        ...
        all_created_files.append((png_path, png_path.replace(OUTPUT_PNG_DIR + "/", "")))
        
    # Generate section PDF file
    pdf_path = os.path.join(OUTPUT_PDF_DIR, f"{section_name}_printable.pdf")
    ...
    all_created_files.append((pdf_path, f"printable_qrs/{section_name}_printable.pdf"))

# ── 4. Create ZIP package containing PNGs and PDFs ───────────────────────────
print(f"\n📦 Packaging all assets into ZIP file: {ZIP_FILE}")
with zipfile.ZipFile(ZIP_FILE, "w", zipfile.ZIP_DEFLATED) as zf:
    for local_path, arc_path in all_created_files:
        zf.write(local_path, arc_path)

print(f"\n✅ All Done! Processed {total_parsed} students.")
print(f"   ZIP file created: {ZIP_FILE}")
print(f"   Section PDFs saved in: ./{OUTPUT_PDF_DIR}/")
print(f"   Individual PNGs saved in: ./{OUTPUT_PNG_DIR}/")
